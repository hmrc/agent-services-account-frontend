...
object UpdateMoneyLaunderingSupervisionForm {
  ...
  private def invalidDateCheck(
    day: String,
    month: String,
    year: String
  ): Either[(String, String), ValidationResult] = {
    Try {
  ...
  private def pastExpiryDateCheck(
    day: String,
    month: String,
    year: String
  ): Either[(String, String), ValidationResult] = {
    if (
      ...
      Left("day" -> "update-money-laundering-supervisory.error.date.past")
  }

  private def within13MonthsExpiryDateCheck(
    day: String,
    month: String,
    year: String
  ): Either[(String, String), ValidationResult] = {
    ...
    if (
      ...
      Left("day" -> "update-money-laundering-supervisory.error.date.before")
  }

  private def noDateCheck(
    day: String,
    month: String,
    year: String
  ): Either[(String, String), ValidationResult] = {
    if (s"$day$month$year".isEmpty)
      Left("day" -> "update-money-laundering-supervisory.error.date")
    else
      Right(Valid)
  }

  private def noDayAndMonthCheck(
    day: String,
    month: String,
    year: String
  ): Either[(String, String), ValidationResult] = {
  ...
  private def noMonthAndYearCheck(
    day: String,
    month: String,
    year: String
  ): Either[(String, String), ValidationResult] = {
  ...
  implicit val dateFormatter: Formatter[LocalDate] =
      ...
      override def bind(
        ...
        val result =
          for {
            ...
            _ <- pastExpiryDateCheck(
              day,
              month,
              year
            )
            _ <- within13MonthsExpiryDateCheck(
              day,
              month,
              year
            )
            date <- Right(LocalDate.of(
              year.toInt,
              month.toInt,
              day.toInt
            ))
