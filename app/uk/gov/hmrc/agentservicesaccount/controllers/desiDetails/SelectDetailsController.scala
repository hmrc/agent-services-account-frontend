        ...
        sessionCacheService.put(
          ...
          newData.copy(agencyDetails =
            oldContactDetails.copy(
              agencyName =
                if (selectedChanges.businessName.isDefined)
                  newData.agencyDetails.agencyName
                else
                  None,
              agencyAddress =
                if (selectedChanges.address.isDefined)
                  newData.agencyDetails.agencyAddress
                else
                  None,
              agencyEmail =
                if (selectedChanges.email.isDefined)
                  newData.agencyDetails.agencyEmail
                else
                  None,
              agencyTelephone =
                if (selectedChanges.telephone.isDefined)
                  newData.agencyDetails.agencyTelephone
                else
                  None
            )
          )
