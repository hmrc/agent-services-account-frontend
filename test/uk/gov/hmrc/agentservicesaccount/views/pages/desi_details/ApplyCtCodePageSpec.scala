...
import uk.gov.hmrc.agentservicesaccount.forms.UpdateDetailsForms.applyCtCodeChangesForm
...
class ApplyCtCodePageSpec extends BaseISpec {

  implicit val messagesApi: MessagesApi = app.injector.instanceOf[MessagesApi]
  implicit val appConfig: AppConfig = app.injector.instanceOf[AppConfig]
  implicit val lang: Lang = Lang("en")
  val view: apply_ct_code_changes  = app.injector.instanceOf[apply_ct_code_changes]
  implicit val messages: Messages = MessagesImpl(lang, messagesApi)

  val form: Form[ApplyCtCodeChanges] = applyCtCodeChangesForm
  val formWithNameErrors: Form[ApplyCtCodeChanges] = form.withError(
    key ="applyChanges",
    message = Messages("update-contact-details.apply-ct-code-changes.error.empty")
  )

  "apply_ct_code_changes" when {
    ...
    def testPageStaticContent(doc: Document): Unit = {
      ...
      "have the correct h1 heading" in {
        doc.select("h1").first.text() mustBe "Do you want to apply these changes to your Corporation Tax agent code?"
      }
    ...
    "first viewing page" should {
      ...
      "display the correct page title" in {
        doc.title() mustBe "Do you want to apply these changes to your Corporation Tax agent code? - Agent services account - GOV.UK"
      }
    }
    ...
    "form is submitted with error should" should {
      ...
      "display error prefix on page title" in {
        doc.title() mustBe "Error: Do you want to apply these changes to your Corporation Tax agent code? - Agent services account - GOV.UK"
      }

      "display correct error summary link" in {
        val errorLink: Element = doc.select(".govuk-error-summary__list a").first()
        errorLink.text() mustBe "Select yes if you want to apply these changes to your Corporation Tax agent code."
      ...
      "display error message on form" in {
        doc.select(".govuk-error-message").text() mustBe "Error: Select yes if you want to apply these changes to your Corporation Tax agent code."
      }
    }
