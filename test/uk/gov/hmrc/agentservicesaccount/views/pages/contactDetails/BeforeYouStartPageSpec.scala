      ...
      "render correctly" when {
        "the selected lang is english" when {
          ...
          "there are contact details" in {
            val doc: Document = Jsoup.parse(view.apply(testAgencyDetailsFull, isAdmin = true)(FakeRequest(), messages, appConfig).body)
          ...
          "'page can't be accessed for standard user" in {
            val doc: Document = Jsoup.parse(view.apply(testAgencyDetailsFull, isAdmin = false)(FakeRequest(), messages, appConfig).body)
