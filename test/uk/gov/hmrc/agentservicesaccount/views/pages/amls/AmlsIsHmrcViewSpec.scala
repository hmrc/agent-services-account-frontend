  ...
  "is_amls_hmrc view" when {
    ...
    def testPageStaticContent(doc: Document): Unit = {
      ...
      "have the correct h1 heading and legend" in {
        doc.select("h1").first.text() mustBe "Is HMRC your money laundering supervisory body?"
        doc.select("legend").text() mustBe "Is HMRC your money laundering supervisory body?"
      }

      "have the correct radio labels" in {
        doc.select(".govuk-radios label").first().text() mustBe "Yes"
        doc.select(".govuk-radios label").last().text() mustBe "No"
      }
    ...
    "form is submitted with errors should" should {
      ...
      "display error message on form" in {
        doc.select(".govuk-error-message").text() mustBe "Error: Select yes if HMRC is your money laundering supervisory body"
      }
