...
with ScalaFutures {
  ...
  def assertElementInPositionContainsText(
    doc: Document,
    cssSelector: String,
    index: Int = 0,
    expectedText: String
  ...
  def assertAttributeValueForElement(
    element: Element,
  ...
  def assertPageContainsText(
    doc: Document,
    text: String
  ): Assertion = assert(doc.toString.contains(text), "\n\ntext " + text + " was not rendered on the page.\n")

  def assertDoesNotContainText(
    doc: Document,
    text: String
