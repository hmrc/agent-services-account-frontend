...
extends ComponentBaseISpec {
  ...
  private val enterCtCodePath = s"$desiDetailsStartPath/enter-CT-code"

  s"GET $enterCtCodePath" should {
  ...
  s"POST $enterCtCodePath" should {
    ...
    "redirect to /your-details and store data" in {
      ...
      val result = post(enterCtCodePath)(body = Map("ctCode" -> List("AB1234")))

      result.status shouldBe SEE_OTHER

      result.header("Location").get shouldBe s"${desiDetails.routes.YourDetailsController.showPage.url}"
    }
