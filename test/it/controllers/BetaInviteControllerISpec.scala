...
import uk.gov.hmrc.agentservicesaccount.controllers.BetaInviteController
...
class BetaInviteControllerISpec
extends BaseISpec {

  val controller: BetaInviteController = inject[BetaInviteController]
  implicit val mockSessionCacheService: SessionCacheService = inject[SessionCacheService]
  implicit val mockSessionCacheRepo: SessionCacheRepository = inject[SessionCacheRepository]

  override implicit lazy val app: Application = appBuilder(Map("mongodb.uri" -> s"mongodb://localhost:27017/test-BetaInviteControllerSpec"))
    .build()
